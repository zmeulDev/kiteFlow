<div class="space-y-4 lg:space-y-6" wire:key="building-list">
    <!-- Page Header -->
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-xl lg:text-2xl font-bold text-gray-900">Buildings</h1>
            <p class="mt-1 text-sm text-gray-500">Manage buildings in your facility</p>
        </div>
    ...
    <div class="lg:hidden space-y-3">
        ...
        <div class="bg-white rounded-2xl border border-gray-100 p-4 hover:shadow-sm transition-shadow">
            <div class="flex items-start gap-3">
                <div class="w-12 h-12 rounded-xl bg-violet-100 flex items-center justify-center text-violet-600 flex-shrink-0">
                    <i class="fa-solid fa-building text-lg"></i>
                </div>
                <div class="flex-1 min-w-0">
                    <div class="flex items-start justify-between gap-2">
                        <div>
                            <p class="text-sm font-semibold text-gray-900">{{ $building->name }}</p>
                            <p class="text-xs text-gray-500">{{ $building->code ?? 'No Code' }}</p>
                        </div>
                    ...
                    <div class="mt-2 text-xs text-gray-500">
                        ...
                        {{ $building->city }}, {{ $building->country ?? 'N/A' }}
                    </div>
                    <div class="mt-3 flex items-center gap-2">
                        <button wire:click="openEditModal({{ $building->id }})" 
                        ...
                        <button wire:click="openDeleteModal({{ $building->id }})" 
        ...
        <div class="bg-white rounded-2xl border border-gray-100 p-8 text-center">
            ...
            <p class="text-sm font-semibold text-gray-900">No buildings found</p>
            <p class="text-sm text-gray-500 mt-1">Add your first building</p>
        </div>
        @endforelse
        ...
        <div class="bg-white rounded-2xl border border-gray-100 p-4">
            <div class="flex items-center justify-between">
                <p class="text-xs text-gray-500">{{ $buildings->firstItem() }}-{{ $buildings->lastItem() }} of {{ $buildings->total() }}</p>
    ...
    <div class="hidden lg:block bg-white rounded-2xl border border-gray-100 overflow-hidden">
            ...
            <table class="w-full">
                <thead>
                    <tr class="bg-gray-50 border-b border-gray-100">
                        ...
                        <th class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wide">Address</th>
                    ...
                    <tr class="hover:bg-gray-50/50 transition-colors">
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <p class="text-sm font-semibold text-gray-900">{{ $building->name }}</p>
                                    <p class="text-xs text-gray-400">{{ $building->code ?? '-' }}</p>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4">
                            <p class="text-sm text-gray-600">{{ $building->address ?? '-' }}</p>
                            <p class="text-xs text-gray-400">{{ $building->city }}, {{ $building->country ?? '' }}</p>
                        </td>
                        <td class="px-6 py-4">
                            <p class="text-sm text-gray-600">{{ $building->email ?? '-' }}</p>
                            <p class="text-xs text-gray-400">{{ $building->phone ?? '-' }}</p>
                        </td>
                        <td class="px-6 py-4">
                            @if($building->is_active)
                            <span class="inline-flex items-center gap-1.5 px-2.5 py-1 text-xs font-semibold rounded-full bg-emerald-50 text-emerald-700">
        ...
        <div class="flex items-center justify-between px-6 py-4 border-t border-gray-100">
            <p class="text-sm text-gray-500">Showing {{ $buildings->firstItem() }} to {{ $buildings->lastItem() }} of {{ $buildings->total() }}</p>
            {{ $buildings->links() }}
        </div>
        @endif
    </div>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4 overflow-y-auto" x-data>
        ...
        <div class="relative w-full max-w-lg bg-white rounded-2xl shadow-xl my-8 max-h-[90vh] overflow-y-auto">
            <form wire:submit="save">
                <div class="flex items-center justify-between p-5 border-b border-gray-100 sticky top-0 bg-white rounded-t-2xl">
                    <h2 class="text-lg font-semibold text-gray-900">{{ $selectedBuilding ? 'Edit Building' : 'Add Building' }}</h2>
                ...
                <div class="p-5 space-y-4">
                    ...
                    <div class="flex items-center gap-2">
                        <input type="checkbox" wire:model="is_active" id="building_is_active" 
                               class="w-4 h-4 rounded border-gray-300 text-brand-600 focus:ring-brand-500">
                        <label for="building_is_active" class="text-sm text-gray-700">Active</label>
                    </div>
                </div>
                
                <div class="flex items-center justify-end gap-3 p-5 border-t border-gray-100 bg-gray-50 rounded-b-2xl sticky bottom-0">
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4 overflow-y-auto" x-data>
        ...
        <div class="relative w-full max-w-md bg-white rounded-2xl shadow-xl my-8">
            <div class="p-5 text-center">
                ...
                <h3 class="text-lg font-semibold text-gray-900">Delete Building?</h3>
                <p class="text-sm text-gray-500 mt-2">This action cannot be undone. The building will be permanently removed.</p>
            </div>
