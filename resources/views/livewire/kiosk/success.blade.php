    ...
    <div class="card kiosk-card">
        ...
        <div class="bg-main rounded-lg p-5 mb-6 text-left">
            ...
            <div class="kiosk-detail-row">
                ...
                <span>{{ $visit->visitor->company->name ?? 'N/A' }}</span>
            </div>
            <div class="kiosk-detail-row">
                <span class="font-semibold text-secondary">Host</span>
                <span>{{ $visit->host?->name ?? $visit->host_name }}</span>
            </div>
            <div class="kiosk-detail-row">
            ...
            <div class="kiosk-detail-row">
                ...
                <span>{{ $visit->entrance->name }}</span>
            </div>
            <div class="kiosk-detail-row">
                <span class="font-semibold text-secondary">Building</span>
                <span>{{ $visit->entrance->building->name }}</span>
            </div>
            <div class="kiosk-detail-row">
                <span class="font-semibold text-secondary">Check-in Time</span>
                <span>{{ $visit->check_in_at->format('F j, Y g:i A') }}</span>
            </div>
        </div>
